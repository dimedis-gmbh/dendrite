...
test.describe.serial('Dendrite File Manager', () => {
    ...
    test('should support browser back button and URL synchronization', async ({page}) => {
        ...
        // Use browser back button to go back to root
        await page.goBack();
        ...
        await expect(page).toHaveURL(/\/$/);
        await expect(page.locator('#path-display')).toContainText('/');
        await expect(page.locator('text=sample.txt')).toBeVisible(); // Should see root files

        // Use browser forward button to go back to subfolder
        await page.goForward();

        // Should be back in subfolder
        await page.waitForTimeout(2000);
        await expect(page).toHaveURL(/\/subfolder\/?$/, {timeout: 5000});
        await expect(page.locator('#path-display')).toContainText('subfolder');
    });
    ...
    test('should copy and paste file between folders without errors', async ({page}) => {
        ...
        const subfolderDir = page.locator('.file-row').filter({hasText: 'subfolder'});
        await expect(subfolderDir).toBeVisible({timeout: 10000});
        await subfolderDir.dblclick();
